...
void MessageQueue::processMessages(Timer::nanoseconds_float max_processing_time)
{
	...
	{
		std::cout << "INFO:\t MessageQueue took " << std::fixed << std::setprecision(2) << std::setfill('0') << std::setw(3) << timer.getElapsedTime<Timer::milliseconds>() << "ms processing " << std::setw(4) << processedPriorityMessages << " priority messages.\n";
	}
	...
	{
		std::cout << "INFO:\t MessageQueue took " << std::fixed << std::setprecision(2) << std::setfill('0') << std::setw(3) << timer.getElapsedTime<Timer::milliseconds>() << "ms processing " << std::setw(4) << processedMessages + processedPriorityMessages << " messages. " << std::setw(4) << message_queue.size() << " remain\n";
	}
}
