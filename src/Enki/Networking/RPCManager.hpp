...
public:
	...
	//global rpc
	[[nodiscard]] RPCType getGlobalRPCType(const std::string& name) const;

	//entity rpc
	[[nodiscard]] RPCType getEntityRPCType(HashedID type, const std::string& name) const;

	//class rpc
	template <typename T>
	[[nodiscard]] RPCType getClassRPCType(const std::string& name) const;
