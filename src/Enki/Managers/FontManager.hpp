...
namespace enki
{
	...
	public:
		...
		[[nodiscard]]
		sf::Font* getFont(HashedID name) const;

		[[nodiscard]]
		std::string getFontPath(HashedID name) const;
		...
		template <size_t N>
		[[nodiscard]]
		sf::Font* getFont(const char(&name)[N]) const;

		template <size_t N>
		[[nodiscard]]
		std::string getFontPath(const char(&name)[N]) const;
		...
		else
		{
			...
			else //the font name hasn't been added before due to mixing this call vs the other
			{
				spdlog::error(
					...
					font_name_it->second, path, hash(name));
			}
		}
