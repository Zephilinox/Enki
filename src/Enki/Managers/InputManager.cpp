...
bool InputManager::isKeyUp(Keyboard::Key key)
{
	...
	return (keys_this_frame[k] == InputState::Unheld || keys_this_frame[k] == InputState::JustReleased);
}
...
{
	const auto k = static_cast<std::size_t>(key);
	return (keys_this_frame[k] == InputState::HeldDown || keys_this_frame[k] == InputState::JustPressed);
}

bool InputManager::isMouseButtonUp(Mouse::Button button)
{
	const auto b = static_cast<std::size_t>(button);
	return (mouse_buttons_this_frame[b] == InputState::Unheld || mouse_buttons_this_frame[b] == InputState::JustReleased);
}
...
{
	const auto b = static_cast<std::size_t>(button);
	return (mouse_buttons_this_frame[b] == InputState::HeldDown || mouse_buttons_this_frame[b] == InputState::JustPressed);
}
