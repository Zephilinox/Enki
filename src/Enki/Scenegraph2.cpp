	...
	const std::vector<EntityChildCreationInfo>& children)
{
	...
	if (parentID == 0)
	{
		entitiesParentless.push_back(entity->info.ID);
	}
...
{
	for (auto id : entitiesParentless)
	{
		update(dt, id);
	}
}
...
std::vector<Entity*> Scenetree::findEntitiesByParent(EntityID parent) const
{
	...
	for (const auto& [version, entity] : entitiesLocal)
	{
		if (entity->info.parentID == parent)
		{
			ents.push_back(entity.get());
		}
	}
	...
	Packet& p)
{
	...
	{
		auto e = registeredTypes[info.type](info, this);
		entity = e.get();
