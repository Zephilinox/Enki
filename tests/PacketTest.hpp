...
TEST_CASE("Packet - Custom Vector2 Class")
{
	enki::Packet p;
	vector2 v{ 1, 3 };
	p << v;
...
TEST_CASE("Packet - Write Bits Overflowing Bytes")
{
	enki::Packet p;
	int num = 0b000001111;
...
TEST_CASE("Packet - Read Bits Overflowing Bytes")
{
	enki::Packet p;
	p << 1000000;
...
TEST_CASE("Packet - Compressed floats")
{
	enki::Packet p;
	float f1 = 0.5f;
	...
	float f4 = p2.readCompressedFloat(-10, 10, 0.01f);
	...
	float f6 = p3.readCompressedFloat(0, 1, 0.001f);
	
	CHECK(f5 != f6);
	CHECK(f6 == 0.017f);
}

TEST_CASE("Packet - Practical Test")
{
